...
const fetchMachineStats = async (timeRange: string) => {
  ...
  const params = {
    ...
    interval: timeRange === "1h" ? "10m" : timeRange === "6h" ? "30m" : timeRange === "24h" ? "1h" : timeRange === "7d" ? "6h" : "1d",
  };
...
const MachineStatsPage = () => {
  ...
  const [selectedMetric, setSelectedMetric] = useState<"tokens" | "cost" | "requests">("tokens");
  ...
  const performanceRadarChart = useMemo(() => {
    ...
    return {
      ...
      datasets: topMachines.map((machine, index) => {
        ...
        return {
          label: machineInfo?.name || machineInfo?.network_ip || `Machine ${machine.machine_id}`,
          data: [
            (machine.total_tokens / maxTokens) * 100,
            (machine.request_count / maxRequests) * 100,
            maxResponseTime > 0 ? ((maxResponseTime - machine.avg_response_time) / maxResponseTime) * 100 : 0,
  ...
  const timeSeriesChart = useMemo(() => {
    ...
    return {
      ...
      datasets: topMachines.map((machine, index) => {
        const machineInfo = machineMap.get(machine.machine_id);
        const dataMap = new Map(machine.time_series.map(ts => [
          ts.timestamp,
          selectedMetric === "tokens" ? ts.tokens :
          selectedMetric === "cost" ? ts.cost :
          ts.requests
        ]));
  ...
  const exportData = (format: 'json' | 'csv') => {
    ...
    if (format === 'json') {
      ...
      // Create CSV
      const headers = ['Machine ID', 'Machine Name', 'Total Tokens', 'Total Cost', 'Requests', 'Avg Response Time', 'Avg TTFT'];
      const machineMap = new Map(machines?.map(m => [m.id.toString(), m]) || []);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-3 py-2 text-sm bg-white text-gray-700 hover:bg-gray-50 rounded-lg transition-all shadow-sm border border-gray-200 flex items-center gap-2"
              ...
              <button
                onClick={() => refetch()}
                disabled={isFetching}
                className="p-2 text-gray-500 hover:text-gray-700 hover:bg-white rounded-lg transition-all shadow-sm border border-gray-200"
                title="Refresh"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <h3 className="text-sm text-gray-600 mb-1">Efficiency</h3>
            <p className="text-3xl font-bold text-gray-900">
              {statsData?.totals.requests && statsData?.totals.tokens
                ? Math.round(statsData.totals.tokens / statsData.totals.requests)
                : 0}
              <span className="text-sm text-gray-500 font-normal"> tokens/req</span>
            </p>
          </div>
        </div>
